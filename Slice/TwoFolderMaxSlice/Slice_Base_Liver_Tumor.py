...
class Liver_Tumor_Operations:
    ...
    @staticmethod
    def tumor_linear_enhancement_only_tumor(liver_image=None, tumor_image=None):
        pixel_sum = np.sum(tumor_image)
        pixel_num = np.sum(tumor_image != 0)
        average_tumor_pixel_value = (pixel_sum * 1.0) / (pixel_num * 1.0)
        tumor_image = np.asarray(tumor_image, np.float32)
        tumor_image = tumor_image * tumor_image
        tumor_image = tumor_image / average_tumor_pixel_value
        return tumor_image
class Slice_Base_Liver_Tumor(Slice_Base):
    def __init__(self, path, operation=Liver_Tumor_Operations.tumor_linear_enhancement, size=[256, 256]):
        ...
        for i, diff_size_phase_lesion in enumerate(self.rois):
            for j, diff_phase_lesion in enumerate(diff_size_phase_lesion):
                for z, single_phase_lesion in enumerate(diff_phase_lesion):
                    ...
                    #　self.rois[i, j, z] = enhance_edge(self.rois[i, j, z])
    ...
    def extract_roi_resize(self, new_size):
        ...
        for index, diff_phase_liver in enumerate(self.livers):
            ...
            for index_phase, single_phase_liver in enumerate(diff_phase_liver):
                single_phase_lession = self.do_operation_liver_tumor(liver_image=single_phase_liver, tumor_image=self.lesions[index][index_phase])
    ...
    def extract_roi_diff_size(self, new_sizes=[[45, 45], [20, 20], [100, 100]]):
        ...
        for index, diff_phase_liver in enumerate(self.livers):
            ...
            for new_size in new_sizes:
                new_diff_phase_lession = []
                for index_phase, single_phase_liver in enumerate(diff_phase_liver):
                    single_phase_lesion = self.do_operation_liver_tumor(liver_image=single_phase_liver,
                                                                        tumor_image=self.lesions[index][index_phase])
...
class SliceBaseLiverTumorDataset():
    def __init__(self, config):
        self.traindataset = Slice_Base_Liver_Tumor(config.TRAIN_IMAGE_DIR)
        self.valdataset = Slice_Base_Liver_Tumor(config.VALIDATION_IMAGE_DIR)

    def get_train_next_batch(self, batch_size, distribution):
        return self.traindataset.get_next_batch(batch_size, distribution)

    def get_val_next_batch(self, batch_size, distribution):
        return self.valdataset.get_next_batch(batch_size, distribution)

    def get_val_all_data(self):
        return self.valdataset.get_all_images_labels()

    @staticmethod
    def test(config):
        dataset = SliceBaseLiverTumorDataset(config)
        images, labels = dataset.get_train_next_batch(20, None)
        for diffsizephaseimage in images:
            for diffphaseimages in diffsizephaseimage:
                for singlephaseimage in diffphaseimages:
                    print np.shape(singlephaseimage)
        print labels
        _, labels = dataset.get_val_next_batch(20, None)
        print labels
if __name__ == '__main__':
    # Slice_Base_Liver_Tumor(Config, Liver_Tumor_Operations.tumor_linear_enhancement).unit_test()
    SliceBaseLiverTumorDataset.test(Config)