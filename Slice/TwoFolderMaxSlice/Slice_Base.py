...
class Slice_Base:
    def __init__(self, path):
        ...
        self.epoch_num = 0
        self.start_index = 0
        self.livers, self.lesions, self.labels = Slice_Base.load_lesions_labels(path)
    ...
    def get_next_batch(self, batch_size, distribution=None):
        ...
        if distribution is None:
            if end_index >= len(self.rois):
                ...
                labels.extend(
                    self.labels[self.start_index: len(self.labels)]
                )
                labels.extend(
                    self.labels[:end_index - len(self.labels)]
                )
