...
class DataSet:
    ...
    def resize_images(images, size, rescale=True):
        ...
        return res
    @staticmethod
    def generate_paths(dir_name, cross_ids, target_labels=[0, 1, 2, 3], shuffle=True):
        '''
        ...
        for cross_id in cross_ids:
            cur_dir = os.path.join(dir_name, str(cross_id))
            print cur_dir
    ...
    def __init__(self, data_dir, state, pre_load=False, divied_liver=False,cross_ids=[1, 2, 3],rescale=True, expand_is_roi=False, full_roi_path=None):
        '''
        DataSet的初始化函数
...
def train(logits, images_tensor, expand_images_tensor, labels_tensor, is_training_tensor, cross_id, save_model_path=None, step_width=100):
    cross_ids = [0, 1, 2]
    del cross_ids[cross_id]
    if save_model_path is not None:
        save_model_path = os.path.join(save_model_path, str(cross_id))
