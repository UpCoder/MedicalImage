...
class DataSet:
    ...
    def __init__(self, data_dir, state, pre_load=False, divied_liver=False, rescale=True, expand_is_roi=False, full_roi_path=None):
        ...
        '''
        self.paths, self.labels = DataSet.generate_paths(
            data_dir,
        )
    ...
    def get_next_batch(self, batch_size):
        while True:
            ...
            if end_index > len(self.paths):
                self.epoch_num += 1
                cur_paths.extend(self.paths[self.start_index: len(self.paths)])
                cur_paths.extend(self.paths[:end_index - len(self.paths)])

                cur_labels.extend(self.labels[self.start_index: len(self.paths)])
                cur_labels.extend(self.labels[:end_index - len(self.paths)])
                self.start_index = end_index - len(self.paths)
                print 'state: ', self.state, ' epoch: ', self.epoch_num

            else:
                cur_paths.extend(self.paths[self.start_index: end_index])
            ...
            yield cur_roi_images, cur_labels
...
def train(logits, images_tensor, labels_tensor, is_training_tensor, save_model_path=None, step_width=100):
    train_dataset = DataSet('/home/give/Documents/dataset/MedicalImage/MedicalImage/SL_TrainAndVal/ICIP/train', 'train',
    ...
    val_dataset = DataSet('/home/give/Documents/dataset/MedicalImage/MedicalImage/SL_TrainAndVal/ICIP/val', 'val',
                          rescale=True, divied_liver=False, expand_is_roi=True,
                          full_roi_path='/home/give/Documents/dataset/MedicalImage/MedicalImage/SL_TrainAndVal/train')
